...
from Embedding.LKGE.src.train import *
from Embedding.LKGE.src.test import *
from Embedding.LKGE.src.config import *
from Embedding.parse_args import args
from Embedding.LKGE.src.model.DLKGE import TransE as DLKGE_TransE
from Embedding.LKGE.src.model.LKGE import TransE as LKGE_TransE
from Embedding.LKGE.src.model.GEM import TransE as GEM_TransE
from Embedding.LKGE.src.model.EMR import TransE as EMR_TransE
from Embedding.LKGE.src.model.CWR import TransE as CWR_TransE
from Embedding.LKGE.src.model.LAN import TransE as LAN_TransE
from Embedding.LKGE.src.model.PNN import TransE as PNN_TransE
from Embedding.LKGE.src.model.finetune import TransE as finetune_TransE
from Embedding.LKGE.src.model.MEAN import TransE as MEAN_TransE
from Embedding.LKGE.src.model.SI import TransE as SI_TransE
from Embedding.LKGE.src.model.Snapshot_only import TransE as Snapshot_TransE
from Embedding.LKGE.src.model.EWC import TransE as EWC_TransE
from Embedding.LKGE.src.model.retraining import TransE as retraining_TransE
from Embedding.LKGE.src.data_load.KnowledgeGraph import KnowledgeGraph
import shutil
...
class experiment():
    ...
    def prepare(self):
        ...
        '''set data path'''
        self.args.data_path = os.path.join(self.args.data_path, self.args.dataset, 'benchmark', self.args.dataset.lower())
        self.args.save_path = self.args.save_path + self.args.dataset + '-' + self.args.embedding_model + '-' + self.args.kg + '-' + self.args.loss_name

        '''add logging implement to model path for ablation_study'''
        if self.args.kg == 'LKGE' or 'DLKGE':
        ...
        if not os.path.exists(self.args.save_path):
            os.mkdir(self.args.save_path)
        self.args.log_path = self.args.log_path + datetime.now().strftime('%Y%m%d/')
        if not os.path.exists(self.args.log_path):
            os.mkdir(self.args.log_path)
        self.args.log_path = self.args.log_path + self.args.dataset + '-' + self.args.embedding_model + '-' + self.args.kg + '-' + args.loss_name
    ...
    def continual_learning(self):
        ...
        for ss_id in range(int(self.args.snapshot_num)):
            ...
            if self.args.skip_previous == 'True' and self.args.snapshot < int(self.args.snapshot_num) - 1 and self.args.kg in ['Snapshot', 'retraining']:
            ...
            '''training'''
            if ss_id == 0 or self.args.kg not in ['MEAN', 'LAN', 'LKGE'] or (self.args.kg == 'LKGE' and self.args.using_finetune == 'True') or (self.args.kg == 'DLKGE' and self.args.using_finetune == 'True'):
            ...
            if self.args.snapshot < int(self.args.snapshot_num) - 1:
                if self.args.kg in ['Snapshot', 'retraining']:
        ...
        # print(self.model.rel_embeddings)
...
def train_eval_lkge(args):
    # model = torch.load(
    #     '/Users/nizhibin/PycharmProjects/log_fusion/Embedding/LKGE/checkpoint/Apache_Pgsql-TransE-LKGE-Margin-WO_transfer/4model_best.tar',
    #     map_location="cpu")
    # print(model['state_dict'])
    # print(len(model['state_dict']['new_weight_rel_embeddings_weight']))
    ...
    return E.model.ent_embeddings.weight, E.model.rel_embeddings.weight, E.kg.relation2id, E.kg.relation2inv
