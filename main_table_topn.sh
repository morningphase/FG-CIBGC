...
# 创建输出目录
mkdir -p $output_dir
echo "Begin" > "/$output_dir/$algorithm-$dataset-$lr-$emd_dim-$bs-$cluster-topn.txt"
# 记录执行时间
echo "Begin" > $runtime_path
# 循环遍历数据集
                ...
                for k_factor in "${k_factor[@]}"; do
                  for top_n in "${top_n[@]}"; do
                    ...
                    echo "Compute time completed"
                                cd ../Tools
                                command="python3 evaluate.py --dataset $dataset"
                                echo -e "Dataset: $dataset Embedding: $algorithm." >> "../$output_dir/$algorithm-$dataset-$lr-$emd_dim-$bs-$cluster-topn.txt"
                                echo -e "k_factor: $k_factor\ntop_n: $top_n" >> "../$output_dir/$algorithm-$dataset-$lr-$emd_dim-$bs-$cluster-topn.txt"
                                $command >> "../$output_dir/$algorithm-$dataset-$lr-$emd_dim-$bs-$cluster-topn.txt"
                                echo "Single Experiment completed."
                                cd ../Cluster
                                done
